...
import com.litton.ishirmvp.retrofit.BaseSubscriber;
...
public class UserInfoPresenter extends BasePresenter<UserInfoContract.UserInfoView, UserInfoModel> implements UserInfoContract.UserInfoPresenter {
    ...
    @Override
    public void getUsers(Map token) {
        // RxJava  + OkHttp + Retrofit
        // 网络引擎 + OkHttp
        // 显示正在加载中
        getView().onLoading();
        // 耗时 3s
        getModel().getUsers(token).subscribe(new BaseSubscriber<LoginBean>() {
            @Override
            protected void onError(String errorCode, String errorMessage) {
                ...
                getView().onError(errorCode,errorMessage);
            }

            @Override
            public void onNext(LoginBean userInfo) {
                // 成功
